...
export class MenuItemsService {
  create(createMenuItemDto: CreateMenuItemDto) {
    return `This action adds a new menuItem with data: ${JSON.stringify(createMenuItemDto)}`;
  }
  ...
  update(id: number, updateMenuItemDto: UpdateMenuItemDto) {
    return `This action updates a #${id} menuItem with data: ${JSON.stringify(updateMenuItemDto)}`;
  }
